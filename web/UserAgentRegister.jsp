...
<%
    ...
    try
    {
      ...
      ResultSet rs=st.executeQuery("select count(*) from user_agent where username='"+username+"'");
      while(rs.next())
      {
          int count=rs.getInt(1);
          if(count==0)
          {
              int i=st.executeUpdate("insert into user_agent values('"+username+"','"+password+"','"+email+"','"+mobile+"','"+address+"','"+dob+"','"+gender+"','"+location+"','Waiting')");
              if(i>0)
              {
                  %>
